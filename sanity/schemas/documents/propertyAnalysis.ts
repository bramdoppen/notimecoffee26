...
/**
 * PropertyAnalysis — AI-generated scoring and analysis for a property.
...
export const propertyAnalysis = defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      type: 'array',
      description: '13 categorieën: keuken, badkamer, vloeren, schilderwerk, dak, isolatie, ramen, elektra, leidingwerk, CV-ketel, tuin, kozijnen, overig',
      of: [
        {
          type: 'object',
          fields: [
          ...
          preview: {
            ...
            prepare({ category, needed, costMid }) {
              if (needed === 'not_needed') return { title: `✅ ${category}: niet nodig` };
              const emoji = needed === 'required' ? '🔴' : needed === 'recommended' ? '🟡' : '🔵';
              const cost = costMid ? `€${(costMid / 1000).toFixed(0)}k` : '';
  ...
  preview: {
    ...
    prepare({ address, city, score, tier, analyzedAt }) {
      ...
      const date = analyzedAt
        ...
        : '';
      return {
        title: `${tierEmoji} ${score}/100 — ${address || 'Onbekend'}`,
        subtitle: `${city || ''} · ${date}`,
      };
    },
  },
